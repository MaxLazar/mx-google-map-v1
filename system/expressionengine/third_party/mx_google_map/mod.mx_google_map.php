...
class Mx_google_map
{
        ...
        if (preg_match("/" . LD . $variable . ".*?" . RD . "(.*?)" . LD . '\/' . $variable . RD . "/s", $tagdata, $entries)) {
            ...
            if ($channel->enable['pagination'] == TRUE) {
            	if (version_compare(APP_VER, '2.4', '>='))
				{
                    ...
                    $channel->pagination = $this->EE->pagination->create(__CLASS__);
				}else {
	...
	function fetch_cache($identifier = '')
	{
		...
		$cache_file = (($this->EE->config->item('geo_cache_dir')) ? $this->EE->config->item('geo_cache_dir') : APPPATH.'cache/'.$this->geo_cache_dir).md5($tag.$this->uri);
	...
	function save_cache($sql, $identifier = '')
	{
		...
		$cache_dir  = (($this->EE->config->item('geo_cache_dir')) ? $this->EE->config->item('geo_cache_dir') : APPPATH.'cache/'.$this->geo_cache_dir);
    ...
    function js() {
        return (isset($this->EE->session->cache['mx_google_map_js'])) ? $this->EE->session->cache['mx_google_map_js'] : '';
    }
    ...
    {
        $cache_path = (($this->EE->config->item('geo_cache_dir')) ? $this->EE->config->item('geo_cache_dir') : APPPATH.'cache/' . $this->geo_cache_dir);
    ...
    private function _readCache($key)
    {
        ...
        $cache_timeout = (($this->EE->config->item('geo_cache_lifetime')) ? $this->EE->config->item('geo_cache_lifetime') : $this->cache_lifetime) + (rand(0, 10) * 3600);
